...
export default function RestaurantDetail({ restaurantInfo, setType }) {
  ...
  const overallRatings = () => {
    let ratings = []
    for (let i = 0; i < restaurantInfo.rating; i++) {
      ratings.push(
  ...
  return (
    <div>
      <button onClick={() => setType(1)} className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-greeb-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Back</button>
      <div>
